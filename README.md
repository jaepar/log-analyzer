#  Static Log Analyzer (CWE-532)

본 프로젝트는 소스 코드 정적 분석을 통해 로그 파일에 민감한 정보(비밀번호, 인증 토큰 등)가 유출되는 것을 방지하는 보안 도구입니다. 금융 IT 환경에서 발생할 수 있는 데이터 유출 사고(CWE-532)를 예방하기 위해 개발되었습니다.
